// SWPC live layer (#54): the aurora layer, its click readout and the Kp corner, against mocked
...
test('draws the aurora, reads it back on click and shows Kp', async ({ page }) => {
  ...
  await expect(globe).toHaveAttribute('data-aurora-cells', String(AURORA_FIXTURE_CELLS))
...
test('loads the real aurora forecast and Kp @live', async ({ page }) => {
  ...
  await expect(page.locator(GLOBE)).toHaveAttribute('data-aurora-cells', /^\d+$/, { timeout: 20_000 })
