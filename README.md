...
- **Active alerts layer.** Current NWS alerts are drawn as polygons coloured by severity (Extreme, Severe, Moderate, Minor, Unknown). Click a polygon for the event, affected area and effective/expiry times. Alerts that have no geometry (zone-only) cannot be drawn, so they are listed in an overlay instead, five at a time with a "N more" toggle. If the fetch fails or nothing is active, the overlay says so.
